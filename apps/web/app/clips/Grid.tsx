...
export default function Grid({
  ...
  const [overviewPath, setOverviewPath] = useState<string>("/clips");
  const [detailPrefix, setDetailPrefix] = useState<string>("/p");

  useEffect(() => {
    if (typeof window === "undefined") return;
    const host = window.location.host || "";
    const port = window.location.port || "";
    const isClipsHost = host.startsWith("clips.") || port === "3002";
    if (isClipsHost) {
      // Op clips.levendportret.nl is de overview op '/', en detail op '/[slug]'
      setOverviewPath("");
      setDetailPrefix("");
    }
  }, []);
...
function PageLink({ page, children, active, disabled }: { page: number; children: React.ReactNode; active?: boolean; disabled?: boolean }) {
  ...
  return (
    <Link className={cls} href={page === 1 ? "/" : `/clips?page=${page}`}>
      {children}
    </Link>
  );
}
