...
export default function Slideshow({ images, rounded }: { images: { url: string }[]; rounded: boolean }) {
  ...
  return (
    <div className="relative w-full max-w-3xl mx-auto aspect-[16/9] overflow-hidden" style={{}}>
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img src={safe[idx].url} alt="" className={`w-full h-full object-cover ${rounded ? 'rounded-xl' : ''}`} />
      {safe.length > 1 && (
        <>
