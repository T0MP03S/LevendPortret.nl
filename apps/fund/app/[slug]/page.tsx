...
export default async function PublicCompanyPage({ params }: { params: { slug: string } }) {
  ...
  const families = Array.from(new Set([titleFont, bodyFont]))
    .map((f) => `${makeGoogle(f)}:wght@400;700`)
    .join('&family=');
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-6">
      ...
      <header className="space-y-3">
        <h1 className="text-3xl font-bold" style={{ fontFamily: `'${titleFont}', sans-serif` }}>
          {page.company?.name}
        </h1>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {gallery.map((g: any, idx: number) => (
              ...
              <img
                ...
                src={g.url}
                alt=""
                className={`w-full h-40 object-cover ${page.roundedCorners ? 'rounded-lg' : ''}`}
              />
            ))}
          </div>
        )}
