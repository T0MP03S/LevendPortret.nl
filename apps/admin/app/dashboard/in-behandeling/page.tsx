...
export default function InBehandelingPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl md:text-4xl font-bold">In behandeling</h1>
        <p className="mt-2 text-zinc-600">Nieuwe aanmeldingen die wachten op goedkeuring.</p>
      ...
      <div className="rounded-2xl border border-zinc-200 overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="p-6 text-zinc-600">Geen aanmeldingen gevonden.</div>
        ) : (
            ...
            {items.map((it) => (
              <li key={it.id} className="p-4 flex items-center gap-4 justify-between">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <Link href={`/dashboard/aanmelding/${it.id}`} className="px-3 py-1.5 rounded-md border border-zinc-300 hover:bg-zinc-50 text-sm">Details</Link>
                  <button onClick={() => updateStatus(it.id, "REJECT")} className="px-3 py-1.5 rounded-md border border-zinc-300 hover:bg-zinc-50 text-sm">Afwijzen</button>
                  <button onClick={() => updateStatus(it.id, "APPROVE")} className="px-3 py-1.5 rounded-md bg-coral text-white hover:bg-[#e14c61] text-sm">Goedkeuren</button>
                </div>
              </li>
            ))}
          </ul>
